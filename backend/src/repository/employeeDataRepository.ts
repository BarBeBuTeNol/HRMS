...
class EmployeeDataRepository {
  ...
  async saveJobData(data: any) {
    ...
    // 1. Resolve Job Position ID
    let jobPositionId = null;
    if (jobPosition) {
      if (typeof jobPosition === "string" && isNaN(Number(jobPosition))) {
        const [rows] = await pool.query<RowDataPacket[]>(
          "SELECT id FROM job_positions WHERE position_name = ?",
          [jobPosition],
        );
        if (rows.length > 0) {
          jobPositionId = rows[0].id;
        } else {
          const [res] = await pool.query<ResultSetHeader>(
            "INSERT INTO job_positions (position_name, department_id) VALUES (?, ?)",
            [jobPosition, departmentId || null],
          );
          jobPositionId = res.insertId;
        }
      } else {
    ...
    // 2. Insert/Update emp_info with correct column 'position_id'
    await pool.query(
      `INSERT INTO emp_info
            (user_id, emp_code, department_id, position_id, employment_status, work_start_time, work_end_time, hire_date, salary, benefits, performance_review, training_info, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
            ON DUPLICATE KEY UPDATE
            emp_code=VALUES(emp_code), department_id=VALUES(department_id), position_id=VALUES(position_id),
