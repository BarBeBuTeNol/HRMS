...
class ChangeRequestRepository {
    async create(request: Partial<ChangeRequest>) {
        ...
        const [result] = await pool.query<ResultSetHeader>(
            ...
            (requester_id, target_user_id, field_name, old_value, new_value, reason, evidence_path, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, 'Pending', NOW(), NOW())`,
            [requester_id, target_user_id, field_name, old_value, new_value, reason, evidence_path]
        );
