...
class HeadDashboardRepository {
    ...
    async getDepartmentEmployees(deptId: number, search: string = "", position: string = "") {
        const filters: string[] = ["u.department_id = ?"];
        const queryParams: any[] = [deptId];

        if (search) {
            const like = `%${search}%`;
            filters.push(`(u.first_name LIKE ? OR u.last_name LIKE ? OR ei.emp_code LIKE ?)`);
    ...
    async getDepartmentWork(deptId: number) {
        ...
        const [tasks] = await pool.query<RowDataPacket[]>(
            `SELECT ta.id, u.first_name, u.last_name, ta.deadline AS work_date, ta.task_name AS title, 'Task' as type
             FROM task_assignments ta
             JOIN users u ON ta.user_id = u.id
             WHERE u.department_id = ? AND ta.status IN ('Pending', 'In Progress')
    ...
    async getAllProjects() {
        const [projects] = await pool.query<RowDataPacket[]>('SELECT id, project_name FROM projects');
        return projects;
    }

    async getDepartmentTasks(deptId: number, filters: { search?: string, status?: string, priority?: string }) {
        let query = `
    ...
    async createTask(data: any) {
        const { project_id, assigned_to_user_id, task_name, description, priority, deadline, assigned_by_head_id } = data;
        const [result] = await pool.query<any>(
            ...
            [project_id, assigned_to_user_id, task_name, description, priority, deadline, assigned_by_head_id]
        );
        return result;
    }
    ...
    async getReplacementRequests(deptId: number) {
        const [requests] = await pool.query<RowDataPacket[]>(
            `SELECT tr.id, tr.reason, tr.created_at, tr.status,
             ...
             JOIN users u_req ON tr.existing_user_id = u_req.id
    ...
    async updateReplacementRequestStatus(requestId: string, status: string, remarks?: string) {
