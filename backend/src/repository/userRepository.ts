...
class UserRepository {
    ...
    async logAction(userId: number, action: string, details: string, ip?: string, severity: 'Info' | 'Warning' | 'Critical' = 'Info', target?: string, changeRequestId?: number) {
        await pool.query(
            `INSERT INTO user_logs (user_id, action, details, ip_address, severity, target, change_request_id, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`,
            [userId, action, details, ip, severity, target, changeRequestId]
        );
    }
    ...
    async findAllLogs(params: LogFilterParams) {
        ...
        console.log(">>> findAllLogs Called with params:", JSON.stringify(params, null, 2));
        ...
        if (search) {
            const like = `%${search}%`;
            filters.push(`(u.first_name LIKE ? OR u.last_name LIKE ? OR u.username LIKE ? OR ul.action LIKE ? OR ul.details LIKE ? OR ul.target LIKE ?)`);
    ...
    async updateJobPosition(userId: string | number, position: string) {
        ...
        const [rows] = await pool.query<RowDataPacket[]>("SELECT id FROM emp_info WHERE user_id = ? ORDER BY id DESC LIMIT 1", [userId]);
