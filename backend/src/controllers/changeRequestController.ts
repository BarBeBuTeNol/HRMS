...
export const createChangeRequest = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[Notification] New Change Request by User ${requesterId} for User ${targetUserId}`,
    );
...
export const approveChangeRequest = async (req: Request, res: Response) => {
  try {
    ...
    // 1. Self-Approval Block - Commented out for now as per user request to allow testing/single admin use
    ...
    if (
      ...
      // Update users table
      await userRepository.update(targetId.toString(), {
        [fieldName]: newValue,
      });
    } else if (
      [
      ...
      await pool.query(
        `UPDATE user_detail SET ${dbField} = ? WHERE user_id = ?`,
        [newValue, targetId],
      );
    } else if (
      ["salary", "jobPosition", "employmentStatus", "departmentId"].includes(
        fieldName,
      )
    ) {
      const dbFieldMap: any = {
    ...
    await changeRequestRepository.updateStatus(
      Number(id),
      "Approved",
      approverId,
      comment || "Approved",
