...
const formatDate = (dateMsg) => {
  if (!dateMsg) return "";

  let date = new Date(dateMsg);

  // Force UTC parsing for SQL strings "YYYY-MM-DD HH:mm:ss"
  // The system receives UTC time (e.g., 06:00) but requires Local Time (13:00).
  // Parsing as UTC ensures toLocaleString converts 06:00 UTC -> 13:00 Local.
  if (
    typeof dateMsg === "string" &&
    /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(dateMsg)
  ) {
    const [y, m, d, h, min, s] = dateMsg.split(/[- :]/).map(Number);
    ...
    // Fallback: append Z for generic strings to treat as UTC
    const isoString = dateMsg.replace(" ", "T") + "Z";
  ...
  // Return formatted date string (e.g., "22 Jan 2026, 13:21")
